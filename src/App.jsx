...
// --- CONFIGURACIÓN DE NIVELES (AUDITADOS) ---
const SECTION_1_LEVELS = [
  ...
  // CORRECCIÓN NIVEL 9: Se quitó el obstáculo en (3,2) para permitir salida a la derecha
  ...
  // CORRECCIÓN NIVEL 12: Asegurando huecos transitables
  ...
  // CORRECCIÓN NIVEL 18: Se rediseñó el caracol para evitar bloqueos
  { id: 118, title: "Nivel 18: El caracol", start: { x: 0, y: 0 }, goal: { x: 2, y: 2 }, obstacles: [{x:0,y:1},{x:1,y:1},{x:2,y:1},{x:3,y:1},{x:4,y:1}, {x:4,y:2},{x:4,y:3},{x:4,y:4}, {x:0,y:4},{x:1,y:4},{x:2,y:4},{x:3,y:4}, {x:0,y:2},{x:0,y:3}], par: 18 },
...
const ONBOARDING_SLIDES = [
  {
    ...
    icon: Sparkles, color: "text-indigo-500", bgIcon: "bg-indigo-100"
  },
  {
  ...
  },
  {
    ...
    icon: ShoppingCart, color: "text-rose-500", bgIcon: "bg-rose-100"
  }
];
...
export default function App() {
  ...
  const resetLevel = () => {
    ...
    setCollectedCount(0);
    if (currentLevels[currentLevelIndex].timeLimit) setTimeLeft(currentLevels[currentLevelIndex].timeLimit);
    else setTimeLeft(null);
    if (currentLevels[currentLevelIndex].dynamicObstacles) setCurrentObstacles(currentLevels[currentLevelIndex].dynamicObstacles());
    else setCurrentObstacles(currentLevels[currentLevelIndex].obstacles);
  };
  ...
  const calculateReward = () => {
    ...
    if (stars > currentMaxStars) setLevelStars({...levelStars, [level.id]: stars});
  ...
  const handleArrowClick = (dir) => {
    ...
    if (currentSection === 6) {
      ...
      if (itemIndex !== -1) {
        playSound('collect'); newItems.splice(itemIndex, 1);
        setCurrentItems(newItems); setCollectedCount(c => c + 1);
      } else playSound('move');
      setRobotPos({ x: nextX, y: nextY });
      if (nextX === level.goal.x && nextY === level.goal.y) {
        playSound('success'); setStatus('victory_screen'); calculateReward();
      }
      setTimeout(() => setLastEvent(null), 800);
      return;
    }
    if (status === 'running' || status === 'victory_screen') return;
    if (isRecordingTrick) { if (customTrick.length < 5) setCustomTrick([...customTrick, dir]); return; }
    if (pendingIf) {
      if (!pendingIf.lookDir) setPendingIf({ step: 2, lookDir: dir });
      else { setCommands([...commands, { type: 'if', lookDir: pendingIf.lookDir, actionDir: dir }]); setPendingIf(null); setStatus('idle'); }
      return;
    }
    if (commands.length < 24) { 
      if (pendingLoop) { setCommands([...commands, { type: 'loop', times: pendingLoop, dir }]); setPendingLoop(null); }
      else setCommands([...commands, { type: 'single', dir }]);
      setStatus('idle');
    }
  };
  ...
  const addTrickCommand = () => {
    ...
    if (commands.length < 24) {
      if (pendingLoop) { setCommands([...commands, { type: 'loop', times: pendingLoop, dir: 'TRICK' }]); setPendingLoop(null); }
      else setCommands([...commands, { type: 'function' }]);
      setStatus('idle');
    }
  };
  ...
  const removeCommand = (index) => {
    playSound('delete');
    if (status === 'running' || status === 'victory_screen') return;
    const newCommands = [...commands]; newCommands.splice(index, 1); setCommands(newCommands);
  };
  ...
  const runProgram = async () => {
    ...
    if (currentSection === 6) { setStatus('running'); return; }
    if (commands.length === 0) return;
    setStatus('running');
    setRobotPos(level.start);
    let execItems = [...(level.items || [])], execCollectedCount = 0;
    setCurrentItems(execItems); setCollectedCount(0);
    if (level.dynamicObstacles) setCurrentObstacles(level.dynamicObstacles());
    let currentX = level.start.x, currentY = level.start.y, errorOcurred = false;
    await new Promise(r => setTimeout(r, 800));
    for (let i = 0; i < commands.length; i++) {
      setActiveCommandIndex(i);
      const cmd = commands[i], iterations = cmd.type === 'loop' ? cmd.times : 1;
      for (let j = 0; j < iterations; j++) {
        if (cmd.type === 'loop') setActiveLoopIteration(j + 1);
        let stepsToExecute = [];
        if (cmd.type === 'function' || (cmd.type === 'loop' && cmd.dir === 'TRICK')) stepsToExecute = customTrick; 
        else if (cmd.type === 'if') {
          let testX = currentX, testY = currentY;
          if (cmd.lookDir === 'UP') testY -= 1; if (cmd.lookDir === 'DOWN') testY += 1;
          if (cmd.lookDir === 'LEFT') testX -= 1; if (cmd.lookDir === 'RIGHT') testX += 1;
          const isD = level.laserDoor && testX === level.laserDoor.x && testY === level.laserDoor.y;
          const isDClosed = isD && execCollectedCount < (level.items ? level.items.length : 0);
          const isRockAhead = isObstacle(testX, testY) || testX < 0 || testX >= GRID_SIZE || testY < 0 || testY >= GRID_SIZE || isDClosed;
          stepsToExecute = [isRockAhead ? cmd.actionDir : cmd.lookDir];
        } else stepsToExecute = [cmd.dir];
        for (let executeDir of stepsToExecute) {
          let nextX = currentX, nextY = currentY;
          if (executeDir === 'UP') nextY -= 1; if (executeDir === 'DOWN') nextY += 1;
          if (executeDir === 'LEFT') nextX -= 1; if (executeDir === 'RIGHT') nextX += 1;
          const isD = level.laserDoor && nextX === level.laserDoor.x && nextY === level.laserDoor.y;
          const isDClosed = isD && execCollectedCount < (level.items ? level.items.length : 0);
          if (nextX < 0 || nextX >= GRID_SIZE || nextY < 0 || nextY >= GRID_SIZE || isObstacle(nextX, nextY) || isDClosed) {
            errorOcurred = true; break;
          }
          currentX = nextX; currentY = nextY; setRobotPos({ x: currentX, y: currentY });
          const itemIndex = execItems.findIndex(item => item.x === currentX && item.y === currentY);
          if (itemIndex !== -1) {
            playSound('collect'); execItems.splice(itemIndex, 1); execCollectedCount++;
            setCurrentItems([...execItems]); setCollectedCount(execCollectedCount);
          } else playSound('move');
          await new Promise(r => setTimeout(r, 400));
        }
        if (errorOcurred) break;
      }
      setActiveLoopIteration(-1); if (errorOcurred) break;
    }
    setActiveCommandIndex(-1);
    if (errorOcurred) { playSound('error'); setStatus('error'); }
    else if (currentX === level.goal.x && currentY === level.goal.y) { playSound('success'); setStatus('victory_screen'); calculateReward(); }
    else { playSound('error'); setStatus('error'); }
  };
  ...
  const renderGrid = () => {
    ...
    for (let y = 0; y < GRID_SIZE; y++) {
      for (let x = 0; x < GRID_SIZE; x++) {
        const isRobotHere = robotPos.x === x && robotPos.y === y, isGoalHere = level.goal.x === x && level.goal.y === y;
        const isObsHere = isObstacle(x, y), isItemHere = currentItems.some(i => i.x === x && i.y === y);
        const isDoorHere = level.laserDoor && level.laserDoor.x === x && level.laserDoor.y === y;
        const totalItems = level.items ? level.items.length : 0, isDoorOpen = collectedCount >= totalItems;
        cells.push(
          <div key={`${x}-${y}`} className={`w-9 h-9 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center text-xl sm:text-2xl relative transition-all duration-300 ${isObsHere ? 'bg-gradient-to-br from-slate-700 to-slate-900 border-2 border-slate-600 scale-[1.02] z-0' : 'bg-slate-50 shadow-inner border-2 border-slate-200 hover:bg-slate-100 z-0'} ${isGoalHere && !isRobotHere && (!isDoorHere || isDoorOpen) ? 'bg-gradient-to-br from-green-50 to-emerald-100 border-green-300 animate-pulse shadow-[inset_0_0_20px_rgba(52,211,153,0.2)]' : ''} ${isDoorHere && !isDoorOpen ? 'bg-red-500/10 border-red-400 overflow-hidden' : ''} ${isDoorHere && isDoorOpen ? 'bg-green-500/5 border-green-200' : ''}`}>
            {isObsHere && '🪨'}
            ...
            {isDoorHere && !isDoorOpen && (
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <Lock size={16} className="text-red-700 z-10 bg-white/80 backdrop-blur-sm rounded-full p-0.5 border border-red-200" />
              </div>
            )}
  ...
  const renderTerminalCode = () => {
    let lines = [], lineNum = 1;
    if (currentSection === 6) {
      lines.push(<div key="ev1" className="px-2 py-0.5 flex items-center"><span className="text-slate-600 w-6 text-right mr-4 select-none">{lineNum++}</span><div><span className="text-purple-400">int</span> <span className="text-blue-300">bateria</span> = <span className="text-orange-300">100</span>;</div></div>);
      lines.push(<div key="ev2" className="px-2 py-0.5 flex items-center mt-2"><span className="text-slate-600 w-6 text-right mr-4 select-none">{lineNum++}</span><div><span className="text-purple-400">void</span> <span className="text-pink-300">setup</span>() {"{"}</div></div>);
      lines.push(<div key="ev3" className="px-2 py-0.5 flex items-center ml-4"><span className="text-slate-600 w-6 text-right mr-4 select-none">{lineNum++}</span><div><span className="text-yellow-200">attachInterrupt</span>(BTN_UP, moveUp, FALLING);</div></div>);
      lines.push(<div key="ev4" className="px-2 py-0.5 flex items-center mt-2"><span className="text-slate-600 w-6 text-right mr-4 select-none">{lineNum++}</span><div>{"}"}</div></div>);
      if (lastEvent) {
        lines.push(<div key="ev-act" className="px-2 py-0.5 flex items-center bg-white/10 border-l-4 border-red-500 mt-2 event-line"><span className="text-slate-600 w-6 text-right mr-4 select-none">{lineNum++}</span><div><span className="text-purple-400">void</span> <span className="text-pink-300">move{formatDirCode(lastEvent)}</span>() {"{"} robot.<span className="text-yellow-200">move{formatDirCode(lastEvent)}</span>(); {"}"}</div></div>);
      }
      return lines;
    }
    if (customTrick.length > 0) {
      lines.push(<div key="fs" className="px-2 py-0.5 flex items-center mt-1"><span className="text-slate-600 w-6 text-right mr-4 select-none">{lineNum++}</span><div><span className="text-purple-400">void</span> <span className="text-pink-300">miTruco</span>() {"{"}</div></div>);
      customTrick.forEach((t, i) => lines.push(<div key={`tr${i}`} className="px-2 py-0.5 flex items-center ml-4"><span className="text-slate-600 w-6 text-right mr-4 select-none">{lineNum++}</span><div>robot.<span className="text-yellow-200">move{formatDirCode(t)}</span>();</div></div>));
      lines.push(<div key="fe" className="px-2 py-0.5 flex items-center mb-1"><span className="text-slate-600 w-6 text-right mr-4 select-none">{lineNum++}</span><div>{"}"}</div></div>);
    }
    lines.push(<div key="ls" className="px-2 py-0.5 flex items-center mt-1"><span className="text-slate-600 w-6 text-right mr-4 select-none">{lineNum++}</span><div><span className="text-purple-400">void</span> <span className="text-blue-300">loop</span>() {"{"}</div></div>);
    commands.forEach((cmd, idx) => {
      const isActive = idx === activeCommandIndex, bg = isActive ? 'bg-white/10 border-l-4 border-yellow-400' : 'hover:bg-white/5';
      lines.push(<div key={`cmd${idx}`} className={`px-2 py-0.5 flex items-center ml-4 ${bg}`}><span className="text-slate-600 w-6 text-right mr-4 select-none">{lineNum++}</span><div>{cmd.type==='single'?`robot.${formatDirCode(cmd.dir)}();`:cmd.type==='loop'?`for(int i=0; i<${cmd.times}; i++) { robot.${formatDirCode(cmd.dir)}(); }`:`miTruco();`}</div></div>);
    });
    lines.push(<div key="le" className="px-2 py-0.5 flex items-center"><span className="text-slate-600 w-6 text-right mr-4 select-none">{lineNum++}</span><div>{"}"}</div></div>);
    return lines;
  };

  if (currentView === 'onboarding') {
    const slide = ONBOARDING_SLIDES[currentSlide], Icon = slide.icon, isLast = currentSlide === ONBOARDING_SLIDES.length - 1;
    return (
    ...
    return (
      <div className="min-h-[100dvh] bg-slate-900 text-white p-4 flex flex-col items-center">
        ...
        <div className="bg-white/5 p-8 rounded-[2rem] border border-white/10 w-full max-w-md">
          ...
          <div className="flex flex-wrap gap-2 justify-center mb-8">{shopFaces.map((f, i) => (
            <button key={i} onClick={() => { if(f.owned) { playSound('click'); setRobotDesign(d=>({...d, face: f})); } else if(coins >= f.price) { playSound('buy'); setCoins(c=>c-f.price); const nf=[...shopFaces]; nf[i].owned=true; setShopFaces(nf); } }} className={`relative w-12 h-12 rounded-xl flex items-center justify-center text-2xl transition-all ${robotDesign.face.emoji===f.emoji?'bg-amber-400 scale-110 shadow-lg':f.owned?'bg-white/10':'bg-black/40 opacity-50'}`}>{f.emoji}{!f.owned && <div className="absolute -bottom-1 bg-black text-amber-400 text-[8px] px-1 rounded border border-amber-400/50">{f.price}</div>}</button>
          ))}</div>
  ...
  const ModuleCard = ({ sectionNum, title, desc, icon: Icon, bgGradient, ringClass }) => {
    const isExpanded = expandedSection === sectionNum, levels = sectionNum===1?SECTION_1_LEVELS:sectionNum===2?SECTION_2_LEVELS:sectionNum===3?SECTION_3_LEVELS:sectionNum===4?SECTION_4_LEVELS:sectionNum===5?SECTION_5_LEVELS:SECTION_6_LEVELS;
    const starsEarned = levels.reduce((a, l) => a + (levelStars[l.id] || 0), 0), totalPossible = levels.length * 3;
    return (
      <div className={`bg-white rounded-3xl shadow-sm border border-slate-200 transition-all overflow-hidden ${isExpanded?`ring-2 ${ringClass}`:''}`}>
        <div onClick={() => { playSound('click'); setExpandedSection(isExpanded ? null : sectionNum); }} className="p-6 cursor-pointer flex flex-col sm:flex-row items-center gap-6 relative">
          ...
          <div className="text-center sm:text-left"><h2 className="text-xl font-black text-slate-800">Módulo {sectionNum}: {title}</h2><p className="text-slate-400 text-sm font-medium">{desc}</p></div>
        </div>
        <div className={`transition-all duration-500 ${isExpanded ? 'max-h-[500px] opacity-100 p-6 pt-0 border-t border-slate-100' : 'max-h-0 opacity-0'}`}>
          <div className="grid grid-cols-5 gap-2 mt-4">{levels.map((l, i) => {
            const stars = levelStars[l.id] || 0, isLocked = i > 0 && !(levelStars[levels[i-1].id] > 0);
            return (
              <button key={l.id} disabled={isLocked} onClick={() => { playSound('click'); setCurrentSection(sectionNum); setCurrentLevelIndex(i); setCurrentView('game'); }} className={`aspect-square rounded-xl flex flex-col items-center justify-center font-black transition-all ${stars>0?`${bgGradient} text-white`:isLocked?'bg-slate-50 text-slate-200 cursor-not-allowed':'bg-white border-2 border-slate-100 text-slate-400 hover:border-indigo-300 hover:text-indigo-600 shadow-sm'}`}>
                {i+1}<div className="flex mt-1 gap-0.5">{isLocked?<Lock size={10}/>:[1,2,3].map(s=><Star key={s} size={8} className={s<=stars?"fill-amber-300 text-amber-300":"fill-transparent"}/>)}</div>
              </button>
            );
          })}</div>
        </div>
      ...
      <div className="min-h-[100dvh] bg-slate-50 p-4 flex flex-col items-center">
        ...
        <header className="text-center mt-12 mb-10 w-full max-w-2xl">
          ...
          <div onClick={() => setCurrentView('workshop')} className="bg-slate-900 text-white p-4 rounded-3xl shadow-xl flex items-center justify-between cursor-pointer group transition-all hover:scale-[1.02]">
            <div className="flex items-center gap-4"><div className={`w-12 h-12 rounded-xl flex items-center justify-center text-3xl ${robotDesign.color.bg}`}>{robotDesign.face.emoji}</div><div className="text-left"><h3 className="font-black text-amber-400 flex items-center gap-2">Tienda Premium</h3><p className="text-xs text-white/50">Personaliza tu robot</p></div></div>
    ...
    <div className="min-h-[100dvh] bg-slate-50 flex flex-col items-center p-2 sm:p-4">
      {status === 'victory_screen' && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] p-10 max-w-sm w-full text-center shadow-2xl transform scale-in">
            ...
            <div className="flex gap-2"><button onClick={resetLevel} className="flex-1 bg-slate-100 py-4 rounded-xl font-bold">Reintentar</button><button onClick={nextLevel} className="flex-[2] bg-indigo-600 text-white py-4 rounded-xl font-black">Siguiente</button></div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col lg:flex-row gap-6 w-full max-w-5xl items-center lg:items-start flex-1">
        <div id="game-board-area" className="flex flex-col items-center bg-white p-6 rounded-[2rem] shadow-sm border border-slate-200 shrink-0 scroll-mt-4">
          ...
          <div className="grid gap-2 mb-6" style={{ gridTemplateColumns: `repeat(${GRID_SIZE}, minmax(0, 1fr))` }}>{renderGrid()}</div>
          ...
          <div className="w-full max-w-[380px] bg-slate-900 rounded-2xl shadow-xl overflow-hidden font-mono text-[10px] text-left">
            <div className="bg-slate-800 px-4 py-2 flex items-center justify-between border-b border-black/40"><div className="flex gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-rose-500"></div><div className="w-2.5 h-2.5 rounded-full bg-amber-500"></div><div className="w-2.5 h-2.5 rounded-full bg-emerald-500"></div></div><div className="text-slate-500 uppercase tracking-widest text-[9px] font-black">robot.cpp</div><div className="w-10"></div></div>
        ...
        <div className="w-full lg:w-96 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-200 flex flex-col flex-1 min-h-[300px]">
          <h2 className="text-xl font-black text-slate-800 mb-6 flex justify-between items-center tracking-tight">{currentSection === 6 ? "Interrupciones" : "Comandos"}<span className="bg-emerald-50 text-emerald-600 text-[10px] font-black px-2 py-1 rounded-md">Live</span></h2>
          {currentSection === 2 && <div className="mb-4 bg-purple-50 p-4 rounded-2xl border border-purple-100 shadow-sm"><h3 className="text-xs font-black uppercase text-purple-800 mb-3 flex items-center gap-2"><Repeat size={14}/> Bucle</h3><div className="flex gap-2">{[2,3,4].map(n=><button key={n} onClick={() => setPendingLoop(n)} className={`flex-1 py-2 rounded-lg font-black transition-all ${pendingLoop===n?'bg-purple-600 text-white shadow-lg':'bg-white text-purple-600 border border-purple-200'}`}>x{n}</button>)}</div></div>}
          {currentSection === 3 && <div className="mb-4 bg-orange-50 p-4 rounded-2xl border border-orange-100"><h3 className="text-xs font-black uppercase text-orange-800 mb-3 flex items-center gap-2"><Cpu size={14}/> Condicional</h3><button onClick={() => setPendingIf({step:1, lookDir: null})} className="w-full bg-white text-orange-600 py-3 rounded-xl border border-orange-200 font-black flex items-center justify-center gap-2">Instalar Sensor</button></div>}
          <div className="grid grid-cols-2 gap-3 mb-6 mt-auto">
            {['UP', 'DOWN', 'LEFT', 'RIGHT'].map(d=><button key={d} onClick={() => handleArrowClick(d)} className="bg-white border-b-4 border-slate-200 hover:border-indigo-300 hover:text-indigo-600 rounded-2xl p-4 font-black flex items-center justify-center transition-all active:translate-y-1 active:border-b-0"><ArrowIcon dir={d} size={24}/></button>)}
          </div>
          <div className="flex-grow bg-slate-50 rounded-2xl p-4 mb-6 min-h-[120px] shadow-inner overflow-y-auto">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 flex justify-between">Memoria RAM {commands.length}/24 <Trash2 size={12} className="cursor-pointer" onClick={() => setCommands([])}/></h3>
            <div className="flex flex-wrap gap-2">{commands.map((c, i) => (
              <div key={i} onClick={() => removeCommand(i)} className={`relative w-10 h-10 rounded-xl flex items-center justify-center shadow-sm border transition-all ${i===activeCommandIndex?'bg-amber-400 text-white scale-110 shadow-lg':'bg-white text-slate-600 border-slate-100'}`}><ArrowIcon dir={c.dir} size={18}/><span className="absolute -top-1.5 -right-1.5 bg-slate-800 text-white text-[8px] w-4 h-4 rounded-full flex items-center justify-center">{i+1}</span></div>
            ))}</div>
          </div>
          <div className="flex gap-3 mt-auto"><button onClick={resetLevel} className="flex-1 bg-white border py-4 rounded-xl font-black text-slate-400 flex items-center justify-center gap-2 shadow-sm"><RotateCcw size={18}/> Reset</button><button onClick={runProgram} disabled={commands.length===0 && currentSection!==6} className="flex-[2] bg-indigo-600 text-white py-4 rounded-xl font-black flex items-center justify-center gap-2 shadow-xl hover:bg-indigo-700 disabled:opacity-50"><Play size={18}/> {currentSection===6?'ACTIVAR':'COMPILAR'}</button></div>
        </div>
      </div>
    </div>
  );
